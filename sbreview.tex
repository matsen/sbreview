...
Traditionally, our microbiota have been transmitted from mother to infant in the birth canal and by breastfeeding; immune dysfunction has been associated with the disruption of these practices \citep[reviewed in][]{funkhouser2013mom}.
...
Tax2tree uses a heuristic algorithm to reassign sequence taxonomic labels so that they are concordant with a given rooted phylogenetic tree which is annotated as follows.
The first step is to label the internal nodes of the tree to optimize the $F$-measure (the harmonic mean of precision and recall) across clades.
where the taxonomic label of a clade is thought of as a prediction for the descendant sequence taxonomic labels.
The second step reassigns sequence taxonomic labels based on these clade taxonomic labels.
