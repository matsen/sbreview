...
In general, sample normalization has not received nearly the attention that it has in other applications of high throughput sequencing \citep[such as RNA-Seq, e.g.][]{anders2010differential,robinson2010edger}.
The present sample normalization techniques are simply in terms of \textit{rarefaction}, which means uniform sub-sampling, typically without replacement \citep{caporaso2010qiime,schloss2009introducing}.
Such normalization implicitly assumes a model whereby reads are sequenced independently of one another, which as explained is almost certainly not the case.
One might hope to overcome these problems through clustering reads from the ``same'' organism and then performing rarefaction, but as described in the section on OTUs this is a challenge on its own.
\citet{holmes2012dirichlet,la2012hypothesis} use models where read counts are modeled as overdispersed samples of the true distributions.
\citet{o2012phylogenetic} provide a ``UniFrac score normalization curve'' based on a sampling model of community assembly, but more work could be done exploring results under deviation from that model.
...
I am also grateful to Aaron Darling, David Fredricks, Noah Hoffman, Steven Kembel, Connor McCoy, Martin Morgan, and Sujatha Srinivasan for interesting discussions that informed this review, to Noah Hoffman and Bj\"orn Winckler for providing feedback on the manuscript, and to the NIH (R01 HG005966-01) and NSF (1223057) for financial support.
