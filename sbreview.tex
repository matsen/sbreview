...
This collection of microbes living inside and on top of us is called the \textit{human microbiota}, and the ensemble of genes in those microbes is called the \textit{human microbiome}, although usage of these terms varies \citep{Boon2013-mc}.
...
Much of our metabolic interaction with the outside world is mediated by our microbiota, as it has important roles in immune system development, nutrition, and drug metabolism \citep{kau2011human,maurice2013xenobiotics}; our food and drug intake in turn impacts the diversity of microbes present.
Traditionally, our microbiota have been transmitted from mother to infant in the birth canal and by breastfeeding; immune dysfunction has been associated with the interruption of these practices \citep[reviewed in][]{funkhouser2013mom}.
...
This terminology is equivalent to the ``barcoding'' terminology more commonly used for eukaryotes.
...
At least some portion of the rest of the other approximately 99 percent of the metagenome can be taxonomically classified using a diversity of methods described below.
...
Before proceeding, we note that phylogenetics for microbes differs in some important respects from, say, mammal phylogenetics.
Where the phylogenetics researchers for mammals are converging on one or two possible basic structures for the evolution of mammals, much of the deep history of microbes remains obscure.
Inference of this history is complicated by the fact that any tree-like signal in the overall evolution of bacteria is restricted to a small set of so-called ``core'' genes \citep{bapteste2009prokaryotic,Leigh2011-zu,Lang2013-zt}.
...
Tax2tree uses a heuristic algorithm to reassign sequence taxonomic labels so that they are concordant with a given rooted phylogenetic tree as follows.
The first step is to label the internal nodes of the tree to optimize the $F$-measure (the harmonic mean of precision and recall) across clades where the taxonomic label of a clade is thought of as a prediction for the descendant sequence taxonomic labels; the second step reassigns sequence taxonomic labels based on these clade taxonomic labels.
...
Other metagenomic classification techniques have been reviewed by \citet{mande2012classification}; interesting recent progress has been made by \citet{Lanzen2012-ul}, \cite{Koslicki2013-mc}, and \cite{Droge2014-lj}.
...
For example, the most commonly used index is the Simpson index \cite{simpson1949measurement}, which is simply the sum of the squared frequencies across OTUs.
The drawback of applying such a diversity metric to a collection of OTUs is that the overall structure of the diversity is lost, in that two closely related OTUs contribute as much to the diversity measure as two distant ones.
% \citep{hill1973diversity} Diversity and evenness: a unifying notation and its consequences
Phylogenetic diversity metrics, which do take this overall diversity structure into account, are also used and here we will focus on their applications and methods.
...
\newcommand{\pdLegend}{
    ...
    Unweighted PD takes the sum of the branch lengths $\ell$ connecting samples.
    Abundance-weighted measures take a weighted sum of branch lengths where weight is determined in some way by the abundance of the taxa on either side of the branch: if we give edges width according to their weight, the abundance-weighted measure can be thought of as the sum of the areas $A$ of edges.
}
\forarxiv{
...
Abundance-weighted measures commonly weight edges in proportion to abundance, but one can also construct ``partially abundance weighted'' measures by weighting edges by abundances transformed by a less than linear function.
...
The earth-movers distance between two probability distributions in this case can be defined using a physical analogy as the minimum amount of ``work'' required to move piles of dirt (each dirt pile configuration sized in proportion to one probability distribution) in one configuration to another along the tree, where work is defined as mass times distance.
...
There are clear connections between UniFrac and phylogenetic diversity explored by \cite{faith2009cladistic}, who point out related measures in Faith's earlier work.

\newcommand{\unifracLegend}{
    The UniFrac divergence measure \citep[figure adapted from][]{LozuponeKnightUniFrac05}.
    Assume that the sequence data to build the phylogenetic tree is divided into two samples, the green sample and the blue sample, which might represent samples from two locations.
    ...
    Bottom pictorial equation shows the ratios of interest for UniFrac: the branch length unique to one sample (green or blue, but not brown) divided by the total branch length.
...
\newcommand{\dirtpilesLegend}{
    ...
    For this, each probability distribution is considered as a configuration of dirt piles on the tree, and the distance between two such dirt pile configurations is defined to be the minimum amount of physical ``work'' required to move the dirt in one configuration to the other.
}
\forarxiv{
...
\citet{Brown2013-ia} use locality-sensitive hashing to obtain placement more than two orders of magnitude faster than the \textit{pplacer} program of \cite{matsen2010pplacer}.
...
Because a child's initial microbiota is transmitted from the mother \citep[reviewed in][]{funkhouser2013mom}, there is a somewhat equivalent notion of microbiota extinction when the chain is interrupted.
Consequently, \citet{yatsunenko2012human} have explicitly contrasted microbiota development in urban, forest-dwelling, and rural populations, while \citet{tito2012insights} have endeavored to characterize the microbiota from ancient feces.
How might phylogenetic methods be used in these preservation efforts?

Perhaps in part because of the importance of mother-to-child transmission, there are indications of coevolution between microbiota and their hosts.
\citet{ochman2010evolutionary} found identical tree topologies for primate and microbiome evolution.
For the microbiota, they used maximum parsimony such that each column was a microbe and each such entry took discrete states according to how much of that microbe was present.
...
Imagine, for example, that we have a random observation process on the tree equipped with some collection $B$ of observations at the leaves.
Each sample from the process takes a random subset from $B$ and then throws down some number of reads for each observation in that set, the number of which has mean significantly greater than 1.
If the size of $B$ is large compared to the number of sample observations, then two draws will always appear significantly different even though they are from the same underlying process.
...
The presently used approach is \textit{rarefaction}, which means uniform sub-sampling to the number of reads in the lowest abundance sample \citep{caporaso2010qiime,schloss2009introducing}.
...
Human microbiota research has experienced a frenetic rate of expansion over the past decade, and sometimes the hype has outmatched the science.
However, our microbes are here to stay and so is research on them.
Thus we can look forward to the field of human microbiota analysis settling down to a comfortable and mature middle age as an interesting intersection between ecology and medicine.
Phylogenetics has already contributed significantly to research on the human microbiota and will continue to do so.
