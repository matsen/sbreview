...
This is not unreasonable, as for many applications such as UniFrac, the tree is used as a framework to structure the data, and there is a certain amount of flexibility in that framework that will give the same results.
Furthermore, if the underlying data it 16S we can expect some tree problems even with the best methods.

Data sets can be very large.
Size of GreenGenes tree.

The alignment culture seems to be mostly around large, established alignments.
One such alignment is the SILVA alignment \citep{pruesse2007silva,quast2013silva}, which is available as part of a web tool.

Because of the high rate of insertion and deletion
OMG the SILVA alignment is mostly gap.

Fixed alignment inferred with heuristic algorithms.
...
% note there is also http://www.plosone.org/article/info%3Adoi%2F10.1371%2Fjournal.pone.0013963
