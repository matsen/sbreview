...
The first step is to label the internal nodes of the tree to optimize the $F$-measure (the harmonic mean of precision and recall) across clades, where the taxonomic label of a clade is thought of as a prediction for the descendant sequence taxonomic labels.
The second step reassigns sequence taxonomic labels based on these clade taxonomic labels.
Interestingly, tax2tree allows for polyphyletic taxonomic groups as an allowance for either phylogenetic error, lack of resolution of the 16S gene, or taxonomic groups with no evolutionary basis; the method does not attempt to signal the cause of such polyphyletic groups.
...
However, whereas just about every of the hundreds or thousands of 16S surveys applies an OTU-based alpha diversity estimate, only a few involve PD.
...
\newcommand{\unifracLegend}{
    ...
    Assume that the sequence data to build the phylogenetic tree derives from two samples: the green sample and the blue sample.
    ...
    The bottom pictorial equation shows the ratios of interest for UniFrac: the branch length unique to one sample (green or blue, but not brown) divided by the total branch length.
...
The earth-movers distance between two probability distributions in this case can be defined using a physical analogy as the minimum amount of ``work'', defined as mass times distance, required to move probability mass in one distribution to another along the tree.
...
One way to model microbial community assembly is to apply Hubbell's neutral theory \citep{fierer2012animalcules,costello2012application}.
