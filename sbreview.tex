...
Those used to microbial genetics may think this surprising, because the genetic repertoire of microbes may not be so much vertically inherited.
However, \cite{zaneveld}
This approach has recently been taken to its logical conclusion by trying to infer.
...
Metagenomic data is typically used to infer functional information directly.
...
This is the approach taken by the team of Jonathan Eisen, starting first with AMPHORA \cite{wu2008amphora} and subsequently with PhyloSift (in preparation).

In parallel, MLTreeMap.

Because of the diversity of gene repertoire in microbes, the gene sets may have only limited overlap with one another.
One solution is assembly and then phylogenetic analysis.
metAMOS pipeline \cite{treangen2011metamos}.
